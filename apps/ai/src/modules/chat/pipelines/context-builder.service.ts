...
import { connectDB, MessageModel } from "../../../infrastructure/db";
...
function buildSystemPrompt(opts: BuildSystemPromptOptions): string {
  const { companyName, fallbackToAgent, collectUserInfo } = opts;
...
- You MUST always call the "rewrite_and_think" tool before giving your final answer, to plan your approach.`;
  ...
  if (wantsAny) {
    ...
    if (wantsPhone) {
      fields.push("their **phone number** (optional, for callback support)");
    }
...
**Resolution tracking (no chat closure):**

When a user's specific query is clearly resolved, you MUST call the tool \`mark_query_resolved\` with a brief summary and reason.
Then respond normally to the user and explicitly say the query is resolved (e.g. "That should be resolved now.").

Important rules:
- Do NOT close the conversation or imply the chat is over.
- Do NOT use any [RESOLVE: ...] sentinel.
- Only mark a query resolved when the user confirms success or the request is fully completed.
- If the user continues with a new question, treat it as a new query and repeat the process when resolved.`;


  const canEscalate = fallbackToAgent;
...
export async function buildContext(
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      console.log(
        `[Context] Query: "${currentMessage.slice(0, 100)}${currentMessage.length > 100 ? "..." : ""}"`,
      );
      results.forEach((r, i) => {
        console.log(
      ...
      systemPrompt =
        `${buildSystemPrompt({ companyName, fallbackToAgent: canFallback, collectUserInfo })}\n\n` +
  ...
  try {
    ...
    console.log(
      `[History] DB returned ${rawMessages.length} message(s) for conversation ${conversationId}`,
    );
    ...
    for (const m of chronological) {
      ...
      console.log(
        `  [History] ${role.padEnd(9)} | senderId=${String(m.senderId).slice(0, 16).padEnd(16)} | ${String(m.content).slice(0, 80).replace(/\n/g, " ")}`,
      );
    }
  } catch (err) {
  ...
  allMessages.forEach((m, i) =>
    console.log(
      `  [LLM turn ${i + 1}] ${m.role.padEnd(9)} | ${m.content.slice(0, 100).replace(/\n/g, " ")}`,
    ),
  );
